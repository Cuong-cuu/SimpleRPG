...
public class Enemy3 extends Enemy
{
	...
	public Enemy3()
	{
		...
		this.MaxHP =40*Launcher.difficulty;
		this.hp = 40*Launcher.difficulty;
		this.Damage = 40*Launcher.difficulty;
		this.Defense = 5*Launcher.difficulty;
		this.RunningSpeed = 1.0*Launcher.difficulty;
		this.AttackSpeed = 1.0*Launcher.difficulty;
		this.Range = 400;
		...
		walking.setImage(new Image(ImgLink+"walking"+"_"+"enemy"+"_"+"03"+".png"));
		walkingani = new SpriteAnimation(walking, Duration.millis(800), 9, 9, this.getBgwidth(), this.getBgheight(),this.getBgwidth(),this.getBgheight());		
	
		///Attack animation
		attack.setViewport(new Rectangle2D(0, 0, this.getBgwidth(), this.getBgheight()));
		attack.setImage(new Image(ImgLink+"attack"+"_"+"enemy"+"_"+"03"+".png"));
		attackani = new SpriteAnimation(attack, Duration.millis(1000*this.AttackSpeed), 11, 11, this.getBgwidth(), this.getBgheight(),this.getBgwidth(),this.getBgheight());
				
		///Death animation
		death.setViewport(new Rectangle2D(0, 0, this.getBgwidth(), this.getBgheight()));
		death.setImage(new Image(ImgLink+"death"+"_"+"enemy"+"_"+"03"+".png"));
		deathani = new SpriteAnimation(death, Duration.millis(1000), 8, 8, this.getBgwidth(), this.getBgheight(),this.getBgwidth(),this.getBgheight());
				
		///Hurted animation
		hurted.setViewport(new Rectangle2D(0, 0, this.getBgwidth(), this.getBgheight()));
		hurted.setImage(new Image(ImgLink+"hurted"+"_"+"player"+"_"+"03"+".png"));
		...
		spawn.setImage(new Image(ImgLink+"spawn"+"_"+"enemy"+"_"+"03"+".png"));
		spawnani = new SpriteAnimation(spawn,Duration.millis(1000),8, 8, this.getBgwidth(), this.getBgheight(),this.getBgwidth(),this.getBgheight());
	...
	{	
		SetUpAnimation(
				walking,
				new Image(ImgLink+"walking"+"_"+"enemy"+"_"+"03"+".png"),
				walkingani,
				Duration.millis(800),
				5,
				5,
	...
	private void Attack(double directionX,Player player)
	{
		SetUpAnimation(
				attack,
				new Image(ImgLink+"attack"+"_"+"enemy"+"_"+"03"+".png"),
				attackani,
				Duration.millis(1000*AttackSpeed),
				11,
				11,
		...
		attackani.setOnFinished(e->
		{
			...
			double dis = (player.getX()-X) * (player.getX()-X) + (player.getY()-Y) * (player.getY() - Y);
			if(r1.getBoundsInParent().intersects(player.getBox().getBoundsInParent())==true || Range*Range>=dis )
			{
				GameField.getInstance().addEnemybullet(new Bullet(X,Y,player.getX(),player.getY(),Damage,2.0,3,32,32,19,19,"_enemy_03"));
			}
		});
	...
	public void Spawn(double X,double Y)
	{
		...
		SetUpAnimation(
				spawn,
				new Image(ImgLink+"spawn"+"_"+"enemy"+"_"+"03"+".png"),
				spawnani,
	...
	public void Death()
	{
		...
		SetUpAnimation(
				death,
				new Image(ImgLink+"death"+"_"+"enemy"+"_"+"03"+".png"),
				deathani,
				Duration.millis(1000),
				5,
				5,
	...
	public void Hurted(double Damage)
	{
		...
		SetUpAnimation(
				hurted,
				new Image(ImgLink+"hurted"+"_"+"enemy"+"_"+"03"+".png"),
				hurtedani,
				Duration.millis(500),
				3,
				3,
